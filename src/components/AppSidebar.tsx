...
const projectItems = [{ title: "All projects", url: "/projects", icon: LayoutGrid }];
...
export function AppSidebar() {
  ...
  const creditsPercentage = planLimits?.max_credit_pool ? (credits / planLimits.max_credit_pool) * 100 : 0;
  ...
  return (
    <Sidebar className="border-r border-border/50 bg-background/95 backdrop-blur-xl" collapsible="icon">
      <SidebarHeader className="p-4">
        {/* Logo */}
        <div className="flex items-center justify-between mb-5">
          <button onClick={toggleSidebar} className="flex items-center gap-2.5 group cursor-pointer">
            ...
            {!collapsed && <span className="font-bold text-lg text-foreground">Crealy</span>}
          </button>
          {!collapsed && (
            <button
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button className="w-full flex items-center gap-3 px-3.5 py-3 bg-secondary/40 hover:bg-secondary/60 rounded-xl transition-all duration-200 border border-transparent hover:border-border/50">
                ...
                <span className="flex-1 text-left text-sm text-foreground truncate font-medium">{userEmail}</span>
                ...
                <div className="p-3 bg-secondary/30 rounded-xl space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-semibold text-foreground">
                      ...
                      <span className="text-muted-foreground font-normal">/{planLimits?.max_credit_pool || 5}</span>
                    </span>
                  </div>
                  <Progress value={creditsPercentage} className="h-2" />
                  <p className="text-xs text-muted-foreground">+{planLimits?.daily_credits || 5} crédits/jour</p>
                </div>
              ...
              >
                ...
                <DropdownMenuSub>
                  ...
                  <DropdownMenuSubContent className="bg-popover/95 backdrop-blur-xl border border-border rounded-xl shadow-elevated z-50">
                    <DropdownMenuItem className="px-3 py-2.5 cursor-pointer hover:bg-secondary/50 rounded-lg mx-1">
                      Help Center
                    </DropdownMenuItem>
                    <DropdownMenuItem className="px-3 py-2.5 cursor-pointer hover:bg-secondary/50 rounded-lg mx-1">
                      Contact Us
